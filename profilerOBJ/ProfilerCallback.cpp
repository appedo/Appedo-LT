...
HRESULT ProfilerCallback::Init(ProfConfig * pProfConfig)
{
	...
	qInfo->PathName = L".\\private$\\appedoprofile";
	qDest = qInfo->Open(2, 0);

	//MessageBox(HWND_DESKTOP, L"Text here", L"Text here", MB_OK);
	HRESULT hr = S_OK;
...
__forceinline ThreadInfo *ProfilerCallback::GetThreadInfo()
{
	...
	ThreadID threadID = 0;
	//g_pCallbackObject->m_pProfilerInfo->
...
__forceinline void ProfilerCallback::Enter(FunctionID functionID)
{
	...
	g_pCallbackObject->_LogCallTrace(functionID, TRUE);
...
__forceinline void ProfilerCallback::Leave(FunctionID functionID)
{
	...
	ThreadInfo *pThreadInfo = GetThreadInfo();
	g_pCallbackObject->_LogCallTrace(functionID, FALSE);
...
{
	m_dwEventMask = (DWORD)(COR_PRF_MONITOR_ENTERLEAVE) | (DWORD)(COR_PRF_MONITOR_THREADS) | (DWORD)(COR_PRF_MONITOR_MODULE_LOADS) | (DWORD)(COR_PRF_MONITOR_CACHE_SEARCHES);
	...
	m_dwEventMask = (DWORD)(COR_PRF_ALLOWABLE_AFTER_ATTACH) | (DWORD)(COR_PRF_MONITOR_ENTERLEAVE);
}
...
***************************************************************************************/
/* private */
HRESULT ProfilerCallback::_LogCallTrace(FunctionID functionID, bool Enter)
{
	HRESULT hr = NULL;
	if (functionID != NULL)
	{
		...
		if (pFunctionInfo != NULL)
		{
			...
			WCHAR test[MAX_LENGTH];
			
			time_t now = time(0);
			tm *ltm = localtime(&now);
	
			
			_snwprintf_s(test, MAX_LENGTH, MAX_LENGTH - 1, L"%Id %Id %Id '%Id-%Id-%Id %Id:%Id:%Id' %s", myThreadID, GetTickCount(), functionID, ltm->tm_year + 1900, ltm->tm_mon + 1, ltm->tm_mday, ltm->tm_hour, ltm->tm_min, ltm->tm_sec, pFunctionInfo->m_functionName);
			now = NULL;
			ltm = NULL;
			if (Enter == TRUE)
			{
				...
				//{
				//	/*char buff[200];
				//	sprintf(buff, "E %Id %S %Id %Id\n", GetTickCount(), pFunctionInfo->m_functionName, myThreadID, functionID);
				//	SendMessageToUI(buff);*/
				//	LogToAny1("E %Id %S %Id %Id\n", GetTickCount(), pFunctionInfo->m_functionName, myThreadID, functionID);
				//}

				//FunctionInfoTrace *pFunctionInfoTrace = new FunctionInfoTrace(functionID, 0);
				//pFunctionInfoTrace->m_functionName = pFunctionInfo->m_functionName;
				//pFunctionInfoTrace->m_functionSig = pFunctionInfo->m_functionSig;
				//pFunctionInfoTrace->startime = GetTickCount();
				//m_pFunctionTraceTable->AddEntry(pFunctionInfoTrace, functionID);
				//LogToAny1("enter\n");
			...
			else
			{
				...
				/*if (SUCCEEDED(hr))
				{
				LogToAny1("L %Id %S %Id %Id\n", GetTickCount(), pFunctionInfo->m_functionName, myThreadID, functionID);
				}*/

				//if (threadID != NULL)

				//LogToAny1("L %Id ra %S %S\n", g_pCallbackObject->GetThreadInfo()->m_id, pFunctionInfo->m_functionName, GetTickCount());
				//LogToAny1("L %S \n", pFunctionInfo->m_functionName);
				//m_pFunctionTraceTable->DelEntry(functionID);
			}
		}
...
HRESULT ProfilerCallback::GetFullMethodName(FunctionID functionID, LPWSTR wszMethod, int cMethod)
{
	...
	hr = m_pProfilerInfo->GetTokenAndMetaDataFromFunction(functionID, IID_IMetaDataImport, (LPUNKNOWN *)&pIMetaDataImport, &funcToken);
	if (SUCCEEDED(hr))
	{
		mdTypeDef classTypeDef;
		ULONG cchFunction;
...
void ProfilerCallback::LogToAny1(const char *format, ...)
{
	///////////////////////////////////////////////////////////////////////////
	//Synchronize guard(m_criticalSection);
	///////////////////////////////////////////////////////////////////////////  
...
void ProfilerCallback::SendLog(WCHAR* test, WCHAR* type)
{
	///////////////////////////////////////////////////////////////////////////
	//Synchronize guard(m_criticalSection);
	/////////////////////////////////////////////////////////////////////////// 
...
void ProfilerCallback::LogFunction(FunctionID functionID)
{
	...
	////ULONG size;
	/////LPCBYTE address;
	//FunctionInfoTrace *pFunctionInfo = NULL;
	//
	//pFunctionInfo =m_pFunctionTraceTable->Lookup(functionID);
	//
	//if (pFunctionInfo != NULL)
	//{
	//	/*LogToAny1("f %Id ra\n",
	...
	//	//SIZE_T stackTraceId = _StackTraceId();
	//	//if (wcsncmp(pFunctionInfo->m_functionName, L"CAPPP", 5) == 0  ||  wcsncmp(pFunctionInfo->m_functionName, L"ASP.",4) == 0)
	//	//{

	//	LogToAny1("f %Id %S %S %Id\n",
	//		pFunctionInfo->m_internalID,
	//		SanitizeUnicodeName(pFunctionInfo->m_functionName, ARRAY_LEN(pFunctionInfo->m_functionName), L' '),
	//		SanitizeUnicodeName(pFunctionInfo->m_functionSig, ARRAY_LEN(pFunctionInfo->m_functionSig), L'\0'), GetTickCount(),
	//		pFunctionInfo->m_internalID
	//		);
	//}
	/*	ThreadID myThreadID;
	...
	LogToAny1("f %Id %S ra\n",
	pFunctionInfo->m_internalID, myThreadID);
	myThreadID = NULL;
	hr = NULL;*/
	//		
	//		);
