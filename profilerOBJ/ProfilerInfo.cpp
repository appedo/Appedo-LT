...
FunctionInfoTrace::FunctionInfoTrace(FunctionID functionID, SIZE_T internal) :
BaseInfo(functionID, internal)
{
	wcscpy_s(m_functionName, ARRAY_LEN(m_functionName), L"UNKNOWN");
	wcscpy_s(m_functionSig, ARRAY_LEN(m_functionSig), L"");
...
FunctionInfoTrace::~FunctionInfoTrace()
{
} // dtor
    ...
    m_pStackTraceTable( NULL )
{
	...
	m_pFunctionTraceTable = new Table<FunctionInfoTrace *, FunctionID>();
        ...
        if ( pFunctionInfo == NULL )
        {
            ...
            if ( pFunctionInfo != NULL )
            {
				try
				{
					...
					if (wcsncmp(pFunctionInfo->m_functionName, L"MS.Internal.", 12) != 0 
