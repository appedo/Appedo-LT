...
namespace AppedoLT.BusinessLogic
{
    ...
    public class ThrottledStream : Stream
    {
        ...
        protected void Throttle(int bufferSizeInBytes)
        {
            ...
            //Debug.WriteLine("{0}, {1}, {2}", elapsedMilliseconds, CurrentMilliseconds, _start);
            if (elapsedMilliseconds > 0)
            {
                // Calculate the current bps.
                long bps = _byteCount * 1000L / elapsedMilliseconds;
                //Debug.WriteLine("current bps: {0}; max bps: {1}", bps, _maximumBytesPerSecond);
                ...
                if (bps > _maximumBytesPerSecond)
                {
                    ...
                    if (toSleep > 1)
                    {
                        try
                        {
                            //Debug.WriteLine("Waiting for {0} miliseconds", toSleep);
                            // The time to sleep is more then a millisecond, so sleep.
                            //Thread.Sleep(toSleep);
                            Task.Delay(toSleep).Wait();
                        }
            ...
            {
                // Calculate the time to sleep. In case when elapsedtime is zero
                int toSleep = (int)(_byteCount * 1000L / _maximumBytesPerSecond);
                if (toSleep > 1)
                {
                    try
                    {
